 ...
 * 将1任意移动后再取反,或者先取反再移位,可用来对位置0操作。*/
   if (value) {		      // 如果value为1
      bitmap->bits[byte_idx] |= (1 << bit_odd);
   } else {		      // 若为0
      bitmap->bits[byte_idx] &= ~(1 << bit_odd);
   }
}
...
int bitmap_scan(Bitmap* bitmap, uint32_t cnt){
    ...
    while(1){                   
        while( bitmap_scan_test(bitmap, area_start) && area_start / 8 < bitmap->bytes_len) //当这个while顺利结束1、area_start就是第一个0的位置；2、area_start已经越过位图边界
            area_start++;
        if(area_start / 8 >= bitmap->bytes_len)    //上面那个循环跑完可能是area_start已经越过边界，说明此时位图中是全1，那么就没有可分配内存
            return -1;
        area_size = 1;  //来到了这一句说明找到了位图中第一个0，那么此时area_size自然就是1
        while( area_size < cnt ){
            if( (area_start + area_size) / 8 < bitmap->bytes_len ){    //确保下一个要判断的位不超过边界
